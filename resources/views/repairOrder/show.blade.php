    ...
    <div class="mt-4">
        <div class="border-2 rounded-xl p-6 w-full h-full">
            ...
            <form action="{{ route('repairList.setPayPrice', ['id' => $repairOrder->id]) }}" method="POST"
                ...
                @method('PUT')
                <div class="space-y-4">
                    <div class="text-lg font-kanit font-bold">
                        จำนวนเงินที่ต้องชำระ : {{$repairOrder->cost}}
                    </div>
                    @error('cost')
                    ...
                    <div class="text-lg font-kanit">
                        <input type="number" name="cost" placeholder="ระบุราคา"
                               value="{{ old(0, $repairOrder->cost) }}" autocomplete="off"
                               class="my-input-focus w-24 appearance-none">
                        <span>บาท</span>
                    </div>
                    <button class="my-button-confirm">update</button>
                </div>
            </form>
        </div>
    </div>

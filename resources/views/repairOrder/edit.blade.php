        ...
        <div class="border-2 rounded-xl p-4 w-4/5">
            ...
            <form action="{{ route('repairOrder.update', ['repairOrder' => $repairOrder->id ]) }}" method="POST" enctype="multipart/form-data"
                ...
                <div>
                    ...
                    <input type="text" name="color" placeholder="สี" value="{{ old('color', $repairOrder->color) }}" autocomplete="off"
                           class="@if($errors->has("color")) my-input-danger @else my-input-focus @endif w-full">
                </div>

                <div>
                    <label for="model" class="font-bold pt-2">รุ่น</label>
                    @error("model")<span class="ml-4 text-red-500 text-sm">{{$message}}</span>@enderror
                    <input type="text" name="model" placeholder="รุ่น" value="{{ old('model',$repairOrder->model) }}" autocomplete="off"
                           class="@if($errors->has("model")) my-input-danger @else my-input-focus @endif w-full">
                </div>
